...
vi.mock('./image-with-skeleton', () => ({
  default: ({ src, alt, className }: React.ImgHTMLAttributes<HTMLImageElement>) => (
    <img src={src} alt={alt} className={className} data-testid="user-card-image" />
  ),
}));
...
describe('UsersCard', () => {
  ...
  describe('Click Functionality - Card', () => {
    ...
    it('should not call onEdit or onDelete when card is clicked', () => {
      render(<UsersCard {...defaultProps} />);
      fireEvent.click(screen.getByTestId('user-card'));
  ...
  describe('Different User Data', () => {
    it('should display different user information correctly', () => {
      ...
      render(<UsersCard {...defaultProps} user={differentUser} />);
